...
function getWeather() {
  ...
  function success(position) {
    ...
    // const url = `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely,hourly,alerts&appid=${API_key}&lang=ja`;
    fetch(url)
      .then(res => res.json())
      .then(data => {
        ...
        // 最高気温、最低気温
        message += `最高気温：${tempMax}℃\n`;
        message += `最低気温：${tempMin}℃\n`;
